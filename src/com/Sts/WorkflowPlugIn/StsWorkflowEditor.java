...
public class StsWorkflowEditor extends JPanel
{
    protected StsWin3d win3d;
	protected JEditorPane editPane;
	protected HashMap m_controlMap = new HashMap();

	private StsWorkflowEditor(URL editorPageURL, StsWin3d win3d) throws Exception
    {
        this.win3d = win3d;
    ...
    static public StsWorkflowEditor constructor(URL editorPageURL, StsWin3d win3d)
    {
        try
        {
            return new StsWorkflowEditor(editorPageURL, win3d);
        }
        catch(Exception e)
        {
				...
				((JCheckBox)comp).addActionListener(new ActionListener()
                {
					public void actionPerformed(ActionEvent e)
                    {
                        ...
                        if(((JCheckBox)e.getSource()).isSelected())
                            win3d.workflowSelectPanel.addOptionalStep(m_id);
                        else
                            win3d.workflowSelectPanel.removeOptionalStep(m_id);;
					}
				});
			}
         */
