...
public class StsDefineDualSurfaceFracSimVolume extends StsWizardStep
{
    StsDefineDualSurfaceFracSimVolumePanel panel;
    ...
    public StsDefineDualSurfaceFracSimVolume(StsWizard wizard)
    {
        super(wizard);
        panel = new StsDefineDualSurfaceFracSimVolumePanel(wizard, this);
        header = new StsHeaderPanel();
        setPanels(panel, header);
        panel.setPreferredSize(new Dimension(500, 500));
        header.setTitle("Sub-PostStack3d Definition");
        header.setSubtitle("Defining Dual Surface Sub-PostStack3d");
    ...
    public boolean end()
    {
        if(!((StsSubVolumeWizard)wizard).isCanceled())
        {
            if(dualSurfaceSubVolume == null)
                dualSurfaceSubVolume = StsDualSurfaceSubVolume.constructor(subVolumeName, surfaces, topOffset, botOffset, model.getProject().getZDomain(), model.getProject().getSeismicVelocityModel());
