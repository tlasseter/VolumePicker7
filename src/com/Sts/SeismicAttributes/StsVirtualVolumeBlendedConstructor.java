...
public class StsVirtualVolumeBlendedConstructor extends StsVirtualVolumeConstructor
{
    ...
    private StsVirtualVolumeBlendedConstructor(StsModel model, StsSeismicVolume[] volumes, StsVirtualVolume vVolume, boolean isDataFloat, StsProgressPanel panel)
    {
    ...
    static public StsVirtualVolumeBlendedConstructor constructor(StsModel model, StsSeismicVolume[] data, StsVirtualVolume vVolume, boolean isDataFloat, StsProgressPanel panel)
    {
        try
        {
            StsVirtualVolumeBlendedConstructor blendedVVConstructor = new StsVirtualVolumeBlendedConstructor(model, data, vVolume, isDataFloat, panel);
            return blendedVVConstructor;
        }
        catch (Exception e)
        {
    ...
    public boolean doProcessInputBlock(int nBlock, StsMappedBuffer[] inputBuffers)
    {
		...
		if(inputBuffers.length > 1)		
		{
			...
			result =  ((StsBlendedVirtualVolume)virtualVolume).processBlock(volOneDoubles, volTwoDoubles, nInputSamplesPerBlock);
		}
		else
			result =  ((StsBlendedVirtualVolume)virtualVolume).processBlock(volOneDoubles, nInputSamplesPerBlock);						
