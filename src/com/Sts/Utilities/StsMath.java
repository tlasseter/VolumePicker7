...
//			if (mainDebug)System.out.println("Inserted element in null array.");
    ...
    public static final float[] addVectorsNormalize(float[][] vectors, int length)
    {
        return addVectorsNormalize(vectors, length, 1);
    }
    ...
    public static final float[] addVectorsNormalize(float[][] vectors, int length, int min)
    {
        ...
        float[] sumVector = new float[length];
        float sumSq = 0;
        for (int i = 0; i < length; i++)
        {
            for (int n = 0; n < nVectors; n++)
                if (vectors[n] != null) sumVector[i] += vectors[n][i];
            sumSq += sumVector[i] * sumVector[i];
        }
        if (sumSq == 0.0f) return sumVector;
        float vectorLength = (float) Math.sqrt(sumSq);
        for (int n = 0; n < length; n++)
            sumVector[n] /= vectorLength;
        return sumVector;
    }

    public static final float[] addVectorsNormalize(ArrayList<float[]> vectors, int min)
    {
        int nVectors = vectors.size();
        if(nVectors < min) return null;
	    ...
	    {
		    for(float[] v : vectors)
			    if (v != null) sumVector[i] += v[i];
		    sumSq += sumVector[i] * sumVector[i];
	    }
        if (sumSq == 0.0f) return sumVector;
        float vectorLength = (float) Math.sqrt(sumSq);
        for (int i = 0; i < length; i++)
            sumVector[i] /= vectorLength;
        return sumVector;
    }
	...
	{
		return average(values, nullValue, 1);
	}

    public static float average(float[] values, float nullValue, int minNPoints)
    {
	    int nNonNull = 0;
        float sum = 0.0f;
	    ...
	    if(nNonNull < minNPoints) return nullValue;
        return sum / nNonNull;
    }
