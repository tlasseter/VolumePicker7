...
public class StsOriginalTriangulation
{
	...
	*/
	static boolean CircumCircle ( double xp, double yp, double x1, double y1, double x2, double y2, double x3, double y3, XYZ circle)
	{
	...
	*/
	...
	static int Triangulate(int nv, XYZ pxyz[], ITRIANGLE v[])
	{
	 ...
	 public static void main(String[] args)
     {
		 ...
		 XYZ[] xyzPoints = new XYZ[nPoints + 3];
		 int n = 0, i = 0;
		 for(; n < nPoints; n++)
		 	 xyzPoints[n] = new XYZ(points[n]);
		 for(; i < 3; i++, n++)
			 xyzPoints[n] = new XYZ();

		ITRIANGLE[] triangles = new ITRIANGLE[nPoints * 3];

		for(i = 0; i < triangles.length; i++)
			triangles[i] = new ITRIANGLE();

		 int ntri = Triangulate(nPoints, xyzPoints, triangles);

		for(int tt = 0; tt < xyzPoints.length; tt++)
		{
			System.out.println("rect(" + xyzPoints[tt].x + "," + xyzPoints[tt].y + ", 3, 3);");
		}

		for(int tt = 0; tt < ntri; tt++)
		{
			System.out.println("beginShape(TRIANGLES);");
			System.out.println("vertex( " + xyzPoints[triangles[tt].p1].x + "," + xyzPoints[triangles[tt].p1].y + ");");
			System.out.println("vertex( " + xyzPoints[triangles[tt].p2].x + "," + xyzPoints[triangles[tt].p2].y + ");");
			System.out.println("vertex( " + xyzPoints[triangles[tt].p3].x + "," + xyzPoints[triangles[tt].p3].y + ");");
